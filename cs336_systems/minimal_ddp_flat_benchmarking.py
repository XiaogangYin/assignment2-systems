...
def run_gpt(rank: int,
    ...
    for step in range(num_steps):
        ...
        for p in model.parameters():
            grads.append(p.grad)
            parameters.append(p)
        z = torch._utils._flatten_dense_tensors(grads)
        dist.all_reduce(z,  op=dist.ReduceOp.SUM, async_op=False)
        z.div_(world_size)
        new_grads = torch._utils._unflatten_dense_tensors(z, grads)
        for p, new_grad in zip(parameters, new_grads):
            p.grad = new_grad
